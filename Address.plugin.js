...
`The library plugin needed for ${config.info.name} is missing. Please click Download Now to install it.`, {
     ...
     : (([Plugin, Library]) => {
        ...
        const copyIcon = (width, height) => React.createElement('svg', {
            viewBox: '0 0 24 24',
            width,
            height
        ...
        return class Address extends Plugin {
            constructor() {
                ...
                this.normalizeAddress = Utilities.loadData(config.info.name, "normalizeAddress", false);
            }
            async onStart() {
            ...
            getSettingsPanel() {
                return SettingPanel.build(this.saveSettings.bind(this),
                    new Switch("Popup/Toast", "Confirmation/Error message when copying Address", this.showToast, (e) => {
                        this.showToast = e;
                    }),
                    new Switch("Normalize Address", "Replace PTB/Canary links with normal discord.", this.normalizeAddress, (e) => {
                        this.normalizeAddress = e;
                    }))
            }
            saveSettings() {
                Utilities.saveData(config.info.name, "showToast", this.showToast);
                Utilities.saveData(config.info.name, "normalizeAddress", this.normalizeAddress);
            }
        };
