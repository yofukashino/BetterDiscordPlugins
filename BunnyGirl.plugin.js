	...
	: (([Plugin]) => {
		const plugin = (Plugin) => {			
			...
			return class BunnyGirls extends Plugin {
				...
				getGif() {
					let gifNaN = this.randomize();
					fetch('https://g.tenor.com/v1/search?q=bunny%girls&key=ZVWM77CCK1QF&limit=50').then(function (response) {
						return response.json();
						}).then(function (data) {
						gif = Object.entries(data.results)[gifNaN][1];
						}).catch(function (err) {
						// There was an error
