      ...
      : (([Plugin, Library]) => {
          ...
          return class PremiumScreenShare extends Plugin {
            constructor() {
              ...
              this.res1080p = Utilities.loadData(
                config.info.name,
                "res1080p",
                1080
              );
              this.smoothReso = Utilities.loadData(
                config.info.name,
                "smoothReso",
                720
              );
              this.smoothFPS = Utilities.loadData(
                config.info.name,
                "smoothFPS",
                60
              );
              this.betterReso = Utilities.loadData(
                config.info.name,
                "betterReso",
                0
              );
              this.betterFPS = Utilities.loadData(config.info.name, "betterFPS");
            }
            checkForUpdates() {
              try {
            ...
            async initialize() {
              this.fps = [this.fps15, this.fps30, this.fps60].sort(
                this.ascending
              );
              this.resolution = [
                ...[this.res1080p, this.res720p, this.res480p].sort(
                  this.ascending
                ),
                0,
              ];
            ...
            async patchStream() {
              ...
              StreamStore.ApplicationStreamResolutionButtonsWithSuffixLabel = [];
              StreamStore.ApplicationStreamResolutions = {};
              for (const smoothVideo of StreamStore
                .ApplicationStreamPresetValues[1]) {
                smoothVideo.resolution = this.smoothReso;
                smoothVideo.fps = this.smoothFPS;
              }
              for (const betterReadability of StreamStore
                .ApplicationStreamPresetValues[2]) {
