...
module.exports = (() => {
	...
	: (([Plugin, Library]) => {		
		const { WebpackModules, Patcher, DiscordModules, ContextMenu, Settings } = Library;
		...
		return class VoiceChatUtilities extends Plugin {			
			...
			makeMenu(channel){
				...
				let user = DiscordModules.UserStore.getCurrentUser(); //the user
				...
				if (this.voicechatcopyids) {
					...
					if (Permissions.can(DiscordPermissions.MOVE_MEMBERS, channel)) {
						...
						if (this.exceptSelf && getVoiceChannelId() == channel.id && currentChannel.members.length > 1 &&	currentChannel)
						{
							children.push(
								{
					...
					return ContextMenu.buildMenuChildren([
						{
							...
							action: () => { console.log(`Teri Mummy Meri Hoja ${user.username}`) },
			...
			getMoveableChannels(voiceChannels, channelmembers, delaybetweenactions, user) {
				...
				voiceChannels.forEach((channel) => {
					if (!Permissions.can(DiscordPermissions.CONNECT, channel)) return;
					Moveable.push({
						action: async () => {
							for (const member of channelmembers) {
								if (member == user?.id) continue;
