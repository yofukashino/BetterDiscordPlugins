                ...
                DiscordCommands.BUILT_IN_COMMANDS.push({
					...
					try {						
							...
							async(error, response, body) => {
								if (error) {
									console.log(error);
									sendBotMessage.sendBotMessage(channel.id, "can't even find a insult for you dumb piece of cunt!");
								}	
								send ? sendUserMessage.sendMessage(channel.id, {
									content: body,
									tts: false,
									invalidEmojis: [],
									validNonShortcutEmojis: []
								}, undefined, {}) : sendBotMessage.sendBotMessage(channel.id, body);
							});	
